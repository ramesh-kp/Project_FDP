...
from .WiFi_Network import DeviceName
...
"""
""" Original Finger Print Data Code
...
def FingerprintActivationResponse(Activationid, scanning_number,fingerprint_data = ""):
                ...
                Fingerprint_Encryption_Key = DeviceName()
                Fingerprint_Encryption_Id = str(fingerprint_data)
                print("actual fingerprint",Fingerprint_Encryption_Id)
                Fingerprint_Encoded_Data = cryptocode.encrypt(Fingerprint_Encryption_Id,Fingerprint_Encryption_Key)
                print("encoded fingerprint",Fingerprint_Encoded_Data)
                API_ENDPOINT = url = baseurl_read(Fingerprint_Activation_URL)
                data = {"ActivationId":Activationid,"FingerData":Fingerprint_Encoded_Data,"No":scanning_number}
                results = requests.post(url = API_ENDPOINT, data = data)
                response_message = results.text
                app_logger.info(response_message)
#.................
