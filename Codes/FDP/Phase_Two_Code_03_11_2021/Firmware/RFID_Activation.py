...
def RFIDActivationResponse(Activationid ,id):
        ...
        except Exception as ex:Fingerprint_Encryption_Id
...
def RFIDActivationResponse(Activationid ,id):
        try:
                Rfid_Encryption_Key = DeviceName()
                Rfid_Encryption_Id = str(id)
                print("actual rfid",Rfid_Encryption_Id)
                Rfid_Encoded_Data = cryptocode.encrypt(Rfid_Encryption_Id,Rfid_Encryption_Key)
                print("encoded rfid",Rfid_Encoded_Data)
                app_logger.info(info_log["e30"])
                API_ENDPOINT = baseurl_read(RFID_Activation_Sucess_URL)
                data = {'ActivationId':Activationid,'RFIDCode':Rfid_Encoded_Data }
                r = requests.post(url = API_ENDPOINT, data = data)
                pastebin_url = r.text
                app_logger.info(pastebin_url)
